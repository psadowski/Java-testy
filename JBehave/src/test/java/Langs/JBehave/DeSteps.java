...
public class DeSteps {
    ...
    @Given("ich habe eine Aktion mit dem Symbol $sĂĽmbol und eine Schwelle von $threshold")
    public void aStock(@Named("sĂĽmbol") String symbol, @Named("threshold") double threshold) {
        stock = new Stock(symbol, threshold);
    }

    // Actually it would be spelled "gehandelt" in German, but
    // we want to verify that umlauts work for step names.
    @When("die Aktie zum Preis $price gehĂ¤ndelt wird")
